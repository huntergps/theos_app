...
abstract class CollectionSession with _$CollectionSession {
  ...
  const factory CollectionSession({
    ...
    // Relaciones
    @OdooMany2One('collection.config', odooName: 'config_id') int? configId,
    ...
    @OdooFloat(odooName: 'cash_register_balance_end_real')
    @Default(0.0)
    double cashRegisterBalanceEndReal,
    @OdooFloat(odooName: 'cash_register_balance_end')
    @Default(0.0)
    double cashRegisterBalanceEnd,
    @OdooFloat(odooName: 'cash_register_difference')
    @Default(0.0)
    ...
    @OdooInteger(odooName: 'cheque_recibido_count')
    @Default(0)
    ...
    // Totales monetarios
    @OdooFloat(odooName: 'total_payments_amount')
    @Default(0.0)
    double totalPaymentsAmount,
    @OdooFloat(odooName: 'total_cash_out_amount')
    @Default(0.0)
    double totalCashOutAmount,
    @OdooFloat(odooName: 'total_deposit_amount')
    @Default(0.0)
    double totalDepositAmount,
    @OdooFloat(odooName: 'total_withhold_amount')
    @Default(0.0)
    double totalWithholdAmount,
    @OdooFloat(odooName: 'total_cash_advance_amount')
    @Default(0.0)
    double totalCashAdvanceAmount,

    // Desglose salidas de efectivo
    @OdooFloat(odooName: 'cash_out_security_total')
    @Default(0.0)
    double cashOutSecurityTotal,
    @OdooFloat(odooName: 'cash_out_invoice_total')
    @Default(0.0)
    double cashOutInvoiceTotal,
    @OdooFloat(odooName: 'cash_out_refund_total')
    @Default(0.0)
    double cashOutRefundTotal,
    @OdooFloat(odooName: 'cash_out_withhold_total')
    @Default(0.0)
    double cashOutWithholdTotal,
    @OdooFloat(odooName: 'cash_out_other_total')
    @Default(0.0)
    double cashOutOtherTotal,

    // Totales de cheques
    @OdooFloat(odooName: 'checks_on_day_total')
    @Default(0.0)
    double checksOnDayTotal,
    @OdooFloat(odooName: 'checks_postdated_total')
    @Default(0.0)
    double checksPostdatedTotal,
    @OdooFloat(odooName: 'advance_checks_on_day_total')
    @Default(0.0)
    double advanceChecksOnDayTotal,
    @OdooFloat(odooName: 'advance_checks_postdated_total')
    @Default(0.0)
    double advanceChecksPostdatedTotal,
    @OdooFloat(odooName: 'total_checks_on_day')
    @Default(0.0)
    double totalChecksOnDay,
    @OdooFloat(odooName: 'total_checks_postdated')
    @Default(0.0)
    double totalChecksPostdated,

    // Control de depositos
    @OdooFloat(odooName: 'system_deposits_cash_total')
    @Default(0.0)
    double systemDepositsCashTotal,
    @OdooFloat(odooName: 'manual_deposits_cash_total')
    @Default(0.0)
    double manualDepositsCashTotal,
    @OdooFloat(odooName: 'diff_deposits_cash_total')
    @Default(0.0)
    double diffDepositsCashTotal,
    @OdooFloat(odooName: 'system_deposits_checks_total')
    @Default(0.0)
    double systemDepositsChecksTotal,
    @OdooFloat(odooName: 'manual_deposits_checks_total')
    @Default(0.0)
    ...
    @OdooFloat(odooName: 'total_cash_collected_amount')
    @Default(0.0)
    double totalCashCollectedAmount,
    @OdooFloat(odooName: 'total_cash_pending_amount')
    @Default(0.0)
    double totalCashPendingAmount,
    @OdooFloat(odooName: 'total_credit_orders_amount')
    @Default(0.0)
    double totalCreditOrdersAmount,
    @OdooFloat(odooName: 'total_credit_invoices_amount')
    @Default(0.0)
    double totalCreditInvoicesAmount,
    @OdooFloat(odooName: 'credit_sales_difference')
    @Default(0.0)
    double creditSalesDifference,

    // Conteo manual - Sistema
    @OdooFloat(odooName: 'system_checks_on_day')
    @Default(0.0)
    double systemChecksOnDay,
    @OdooFloat(odooName: 'system_checks_postdated')
    @Default(0.0)
    double systemChecksPostdated,
    @OdooFloat(odooName: 'system_cards_total')
    @Default(0.0)
    double systemCardsTotal,
    @OdooFloat(odooName: 'system_transfers_total')
    @Default(0.0)
    double systemTransfersTotal,
    @OdooFloat(odooName: 'system_advances_total')
    @Default(0.0)
    double systemAdvancesTotal,
    @OdooFloat(odooName: 'system_credit_notes_total')
    @Default(0.0)
    double systemCreditNotesTotal,

    // Conteo manual - Manual
    @OdooFloat(odooName: 'manual_checks_on_day')
    @Default(0.0)
    double manualChecksOnDay,
    @OdooFloat(odooName: 'manual_checks_postdated')
    @Default(0.0)
    double manualChecksPostdated,
    @OdooFloat(odooName: 'manual_cards_total')
    @Default(0.0)
    double manualCardsTotal,
    @OdooFloat(odooName: 'manual_transfers_total')
    @Default(0.0)
    double manualTransfersTotal,
    @OdooFloat(odooName: 'manual_advances_total')
    @Default(0.0)
    double manualAdvancesTotal,
    @OdooFloat(odooName: 'manual_credit_notes_total')
    @Default(0.0)
    double manualCreditNotesTotal,
    @OdooFloat(odooName: 'manual_withholds_total')
    @Default(0.0)
    double manualWithholdsTotal,

    // Conteo manual - Diferencias
    @OdooFloat(odooName: 'diff_checks_on_day')
    @Default(0.0)
    double diffChecksOnDay,
    @OdooFloat(odooName: 'diff_checks_postdated')
    @Default(0.0)
    double diffChecksPostdated,
    @OdooFloat(odooName: 'diff_cards_total')
    @Default(0.0)
    double diffCardsTotal,
    @OdooFloat(odooName: 'diff_transfers_total')
    @Default(0.0)
    double diffTransfersTotal,
    @OdooFloat(odooName: 'diff_advances_total')
    @Default(0.0)
    double diffAdvancesTotal,
    @OdooFloat(odooName: 'diff_credit_notes_total')
    @Default(0.0)
    double diffCreditNotesTotal,
    @OdooFloat(odooName: 'diff_withholds_total')
    @Default(0.0)
    double diffWithholdsTotal,

    // Totales resumen
    @OdooFloat(odooName: 'summary_system_total')
    @Default(0.0)
    double summarySystemTotal,
    @OdooFloat(odooName: 'summary_manual_total')
    @Default(0.0)
    double summaryManualTotal,
    @OdooFloat(odooName: 'summary_diff_total')
    @Default(0.0)
    double summaryDiffTotal,

    // Detalle de Cobros - Depositos
    @OdooFloat(odooName: 'fact_deposits_cash')
    @Default(0.0)
    double factDepositsCash,
    @OdooFloat(odooName: 'fact_deposits_checks')
    @Default(0.0)
    double factDepositsChecks,
    @OdooFloat(odooName: 'cartera_deposits_cash')
    @Default(0.0)
    double carteraDepositsCash,
    @OdooFloat(odooName: 'cartera_deposits_checks')
    @Default(0.0)
    double carteraDepositsChecks,
    @OdooFloat(odooName: 'anticipo_deposits_cash')
    @Default(0.0)
    double anticipoDepositsCash,
    @OdooFloat(odooName: 'anticipo_deposits_checks')
    @Default(0.0)
    double anticipoDepositsChecks,

    // Anticipos cruzados
    @OdooFloat(odooName: 'fact_advances_used')
    @Default(0.0)
    double factAdvancesUsed,
    @OdooFloat(odooName: 'cartera_advances_used')
    @Default(0.0)
    double carteraAdvancesUsed,
    @OdooFloat(odooName: 'summary_advances_used_total')
    @Default(0.0)
    double summaryAdvancesUsedTotal,

    // Total facturas con NC y retenciones
    @OdooFloat(odooName: 'fact_total_with_nc_withholds')
    @Default(0.0)
    ...
    @OdooFloat(odooName: 'fact_checks_post')
    @Default(0.0)
    double factChecksPost,
    ...
    @OdooFloat(odooName: 'cartera_transfers')
    @Default(0.0)
    double carteraTransfers,
    @OdooFloat(odooName: 'cartera_checks_day')
    @Default(0.0)
    double carteraChecksDay,
    @OdooFloat(odooName: 'cartera_checks_post')
    @Default(0.0)
    ...
    @OdooFloat(odooName: 'anticipo_transfers')
    @Default(0.0)
    double anticipoTransfers,
    @OdooFloat(odooName: 'anticipo_checks_day')
    @Default(0.0)
    double anticipoChecksDay,
    @OdooFloat(odooName: 'anticipo_checks_post')
    @Default(0.0)
    ...
    @OdooFloat(odooName: 'total_checks_day')
    @Default(0.0)
    double totalChecksDay,
    @OdooFloat(odooName: 'total_checks_post')
    @Default(0.0)
