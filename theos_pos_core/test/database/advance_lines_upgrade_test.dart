...
void main() {
  for (final from in [10, 11, 12]) {
    test(
      'v$from to v13 adds columns without dropping offline records',
      () async {
        ...
        await db.into(db.accountJournal).insert(
        ...
        final journal = await (db.select(db.accountJournal)
              ..where((table) => table.odooId.equals(901)))
            .getSingle();
