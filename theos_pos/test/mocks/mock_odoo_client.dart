...
class OdooClientMockHelper {
  ...
  void setupCreate({required String model, required int returnId}) {
    when(
      () => client.create(
        model: model,
  ...
  void setupWrite({required String model, bool success = true}) {
    when(
      () => client.write(
        model: model,
        ids: any(named: 'ids'),
  ...
  void setupUnlink({required String model, bool success = true}) {
    when(
      () => client.unlink(
        model: model,
        ids: any(named: 'ids'),
  ...
  void setupSearchCount({required String model, required int count}) {
    when(
      () => client.searchCount(
        model: model,
        domain: any(named: 'domain'),
  ...
  void setupError({required String model, required Exception error}) {
    when(
      () => client.create(
        model: model,
        values: any(named: 'values'),
