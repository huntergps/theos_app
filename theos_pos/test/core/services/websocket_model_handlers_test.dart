...
// ==================== MODEL HANDLER TESTS ====================
// Tests for record notification handlers of all synced models:
// - UomUom (uom.uom)
...
void main() {
  ...
  // ==================== UOM.UOM TESTS ====================
  group('UomUom record notification Handler Tests', () {
    ...
    test('UomUom insert via record notification notification', () async {
      // ACT: Simulate record notification 'created' action - insert new UoM
    ...
    test('UomUom update via record notification notification', () async {
      // ARRANGE
      await insertTestUomUom(
        db,
        odooId: 101,
      ...
      // ACT: Simulate record notification 'updated' action
      await db.customUpdate(
        'UPDATE uom_uom SET name = ?, factor = ? WHERE odoo_id = ?',
        variables: [
          Variable('Docena Modificada'),
          Variable(12.5),
          Variable(101),
        ],
      );
      ...
      final uom = await (db.select(
        db.uomUom,
      )..where((t) => t.odooId.equals(101))).getSingleOrNull();
    ...
    test('UomUom delete via record notification notification', () async {
      // ARRANGE
      await insertTestUomUom(db, odooId: 102, name: 'Caja', categoryId: 1);

      // Verify exists
      var uom = await (db.select(
        db.uomUom,
      )..where((t) => t.odooId.equals(102))).getSingleOrNull();
      expect(uom, isNotNull);

      // ACT: Simulate record notification 'deleted' action
    ...
    test(
      'UomUom deactivation via record notification (active = false)',
      () async {
        // ARRANGE
        await insertTestUomUom(
          db,
        ...
        final uom = await (db.select(
          db.uomUom,
        )..where((t) => t.odooId.equals(103))).getSingleOrNull();
  ...
  // ==================== PRODUCT.UOM TESTS ====================
  group('ProductUom record notification Handler Tests', () {
    ...
    test('ProductUom insert via record notification notification', () async {
      // ARRANGE
      await setupProductUomDependencies(db);

      // ACT: Simulate record notification 'created' action - insert ProductUom
      await db
          .into(db.productUom)
          .insert(
      ...
      // ASSERT
      final productUom = await (db.select(
        db.productUom,
      )..where((t) => t.odooId.equals(100))).getSingleOrNull();
    ...
    test(
      'ProductUom barcode update via record notification notification',
      () async {
        // ARRANGE
        await setupProductUomDependencies(db);
        await db
    ...
    test('ProductUom delete via record notification notification', () async {
      // ARRANGE
      await setupProductUomDependencies(db);
      await db
          .into(db.productUom)
    ...
    test('ProductUom upsert on conflict replaces existing record', () async {
      ...
      await db
          ...
          .insert(
            ...
            onConflict: DoUpdate(
              (old) =>
                  ProductUomCompanion(barcode: const Value('UPSERTED_BARCODE')),
      ...
      final records = await (db.select(
        db.productUom,
      )..where((t) => t.odooId.equals(103))).get();
  ...
  group('ProductPricelistItem record notification Handler Tests', () {
    Future<void> setupPricelistDependencies(AppDatabase db) async {
      // Insert pricelist
      await db
          .into(db.productPricelist)
          .insert(
    ...
    test('PricelistItem insert via record notification notification', () async {
      // ARRANGE
      await setupPricelistDependencies(db);

      // ACT: Insert pricelist item
      await db
          .into(db.productPricelistItem)
          .insert(
    ...
    test(
      'PricelistItem price update via record notification notification',
      () async {
        // ARRANGE
        await setupPricelistDependencies(db);
        await db
  ...
  group('SaleOrderLine record notification Handler Tests', () {
    ...
    test('SaleOrderLine insert via record notification notification', () async {
      // ARRANGE
    ...
    test(
      'SaleOrderLine quantity update via record notification notification',
      () async {
        // ARRANGE
        await setupSaleOrderLineDependencies(db);
        await db
    ...
    test('SaleOrderLine delete via record notification notification', () async {
      // ARRANGE
      await setupSaleOrderLineDependencies(db);
      await db
          .into(db.saleOrderLine)
      ...
      await (db.delete(
        db.saleOrderLine,
      )..where((t) => t.odooId.equals(102))).go();

      // ASSERT
      line = await (db.select(
        db.saleOrderLine,
      )..where((t) => t.odooId.equals(102))).getSingleOrNull();
      expect(line, isNull);
    });

    test('SaleOrderLine discount update via record notification', () async {
      // ARRANGE
      await setupSaleOrderLineDependencies(db);
      await db
          .into(db.saleOrderLine)
  ...
  group('StockByWarehouse record notification Handler Tests', () {
    ...
    test(
      'StockByWarehouse insert via record notification notification',
      () async {
        // ARRANGE
        ...
        // ASSERT
        final stock =
            await (db.select(db.stockByWarehouse)..where(
                  (t) => t.productId.equals(1) & t.warehouseId.equals(1),
                ))
                .getSingleOrNull();
    ...
    test(
      'StockByWarehouse quantity update via record notification notification',
      () async {
        // ARRANGE
        await setupStockDependencies(db);
        await db
    ...
    test('StockByWarehouse multiple warehouses for same product', () async {
      ...
      // ASSERT
      final stocks = await (db.select(
        db.stockByWarehouse,
      )..where((t) => t.productId.equals(1))).get();
    ...
    test('StockByWarehouse zero stock handling', () async {
      ...
      // ASSERT
      final stock =
          await (db.select(db.stockByWarehouse)
  ...
  group('ProductProduct record notification Handler Tests', () {
    ...
    test(
      'ProductProduct insert via record notification notification',
      () async {
        // ARRANGE
    ...
    test(
      'ProductProduct price update via record notification notification',
      () async {
        // ARRANGE
        await setupProductDependencies(db);
        await db
    ...
    test('ProductProduct deactivation via record notification', () async {
      // ARRANGE
      await setupProductDependencies(db);
      await db
          .into(db.productProduct)
    ...
    test('ProductProduct barcode update via record notification', () async {
      // ARRANGE
      await setupProductDependencies(db);
      await db
          .into(db.productProduct)
  ...
  // ==================== RES USERS TESTS ====================
  group('ResUsers record notification Handler Tests', () {
    test('ResUsers insert via record notification notification', () async {
      // ACT
      await db
          .into(db.resUsers)
          .insert(
    ...
    test('ResUsers name update via record notification notification', () async {
      // ARRANGE
      await db
          .into(db.resUsers)
          .insert(
  ...
  // ==================== MAIL ACTIVITY TESTS ====================
  group('MailActivity record notification Handler Tests', () {
    ...
    test('MailActivity insert via record notification notification', () async {
      // ARRANGE
    ...
    test(
      'MailActivity completion via record notification notification',
      () async {
        // ARRANGE
        await setupActivityDependencies(db);
        await db
    ...
    test('MailActivity delete via record notification notification', () async {
      // ARRANGE
      await setupActivityDependencies(db);
      await db
          .into(db.mailActivityTable)
  ...
  group('Edge Cases and Error Scenarios', () {
    test('Handle null values in record notification payload', () async {
      // ARRANGE
      await db
          .into(db.productCategory)
          .insert(
    ...
    test('Handle concurrent updates to same record', () async {
      ...
      // ACT: Simulate rapid updates (like from multiple record notification messages)
      await db.customUpdate(
      ...
      final partner = await (db.select(
        db.resPartner,
      )..where((t) => t.odooId.equals(300))).getSingleOrNull();

      expect(partner, isNotNull);
      expect(partner!.name, 'Final Update');
    });

    test('Handle large text fields in record notification payload', () async {
      // ARRANGE
      ...
      final order = await (db.select(
        db.saleOrder,
      )..where((t) => t.odooId.equals(400))).getSingleOrNull();
    ...
    test('Handle special characters in text fields', () async {
      ...
      final partner = await (db.select(
        db.resPartner,
      )..where((t) => t.odooId.equals(500))).getSingleOrNull();
    ...
    test('Handle decimal precision in price fields', () async {
      ...
      await db
          .into(db.productPricelist)
          .insert(
            ProductPricelistCompanion.insert(odooId: 1, name: 'Precio Público'),
          );
      ...
      final item = await (db.select(
        db.productPricelistItem,
      )..where((t) => t.odooId.equals(600))).getSingleOrNull();
